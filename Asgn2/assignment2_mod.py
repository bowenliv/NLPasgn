...
from imblearn.over_sampling import RandomOverSampler, SMOTENC, SMOTEN, BorderlineSMOTE
...
# two lists (of the same length): the cleaned lines and the party of the person who was speaking
#
...
def read_and_clean_lines(infile):
    ...
    with gzip.open(infile, 'rt') as f:
        for line in tqdm(f):
            ...
            # if line_dict['chamber'] == 'Senate':
            # if line_dict['chamber'] == 'House'
            if line_dict['chamber'] == 'House' or line_dict['chamber'] == 'Senate':
                parties.append(line_dict['party'])
                lines.append(' '.join(line_dict['text'].split()))
...
def split_training_set(lines, labels, test_size=0.3, random_seed=42):
    ...
    # print("Training set label counts: {}".format(Counter(y_train_resampled)))
    # print("Test set     label counts: {}".format(Counter(y_test)))
...
def main(use_sklearn_feature_extraction, num_most_informative, plot_metrics):
    ...
    for label in ['Republican', 'Democrat']:
        print('Precision for label {} = {}'.format(label,
                                                   metrics.precision_score(predicted_labels, y_test, pos_label=label)))
        print('Recall    for label {} = {}'.format(label,
                                                   metrics.recall_score(predicted_labels, y_test, pos_label=label)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_sklearn_features', default=False, action='store_true',
                        help="Use sklearn's feature extraction")
    parser.add_argument('--plot_metrics', default=False, action='store_true', help="Generate figures for evaluation")
    parser.add_argument('--num_most_informative', default=10, action='store',
                        help="Number of most-informative features to show")
